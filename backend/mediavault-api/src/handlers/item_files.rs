...
/// 【機能概要】: `POST /items/:id/files` ハンドラ。path(必須)/label(任意)を受け取り、
/// pathの拡張子からfile_typeを自動分類してitem_filesレコードとして登録する（パス指定方式）
...
/// 直接受け取り、元ファイル名の拡張子からfile_typeを自動分類してファイルサーバーへ書込み、
/// 相対パスをitem_filesへ登録する。
/// 【実装方針】: multipart解析→拡張子からfile_type判定→item存在確認（書込前）→file_storage::store_file()で書込→
...
pub async fn upload_item_file_handler(
    ...
    // 【multipart解析】: file/labelの各パートを読み取る。fileは必須・順不同を許容する。
    // 従来のfile_typeパートは未知フィールドとして無視される（拡張子から自動分類するため） 🔵
    ...
    // 【自動分類】: 元ファイル名の拡張子からfile_typeを判定する（未知拡張子・拡張子なしはother） 🟡
...
mod tests {
    ...
    /// テストケース3: 従来クライアントがfile_type（不正値含む）を送信しても無視され、
    /// 拡張子から自動分類される（ルーター経由・実item必要）
    #[tokio::test]
    #[ignore]
    async fn post_item_file_ignores_legacy_file_type_and_classifies_by_extension() {
    ...
    /// file_typeパートは送信しない（拡張子による自動分類のため）
    ...
    fn multipart_body_without_file() -> (String, Vec<u8>) {
        let boundary = "----mediavaultTestBoundaryTASK0027NoFile";
        let mut body = Vec::new();
        body.extend_from_slice(format!("--{boundary}\r\n").as_bytes());
        body.extend_from_slice(b"Content-Disposition: form-data; name=\"label\"\r\n\r\n");
        body.extend_from_slice("ラベルのみ".as_bytes());
        body.extend_from_slice(b"\r\n");
        body.extend_from_slice(format!("--{boundary}--\r\n").as_bytes());
        (format!("multipart/form-data; boundary={boundary}"), body)
    }
    ...
    async fn insert_test_item(db: &PgPool) -> Uuid {
        sqlx::query_scalar(
            "INSERT INTO items (media_type, title, source) VALUES ('novel', 'TASK-0027テスト書籍', 'manual') RETURNING id",
        )
        .fetch_one(db)
        .await
    ...
    async fn post_item_file_upload_with_pdf_returns_201_and_relative_path() {
        ...
        let (content_type, body) =
            multipart_body(b"%PDF-1.4 dummy bytes", "example.pdf", Some("本編PDF"));
    ...
    async fn post_item_file_upload_with_image_returns_201() {
        ...
        let (content_type, body) =
            multipart_body(b"\xFF\xD8\xFF dummy jpeg", "cover.jpg", Some("表紙"));
    ...
    /// 未知拡張子のアップロードはfile_type=otherとして201で受理される（ルーター経由）
    /// 🟡 信頼性レベル: 拡張子自動分類の設計決定（未知拡張子・拡張子なしはother）より
    #[tokio::test]
    #[ignore]
    async fn post_item_file_upload_with_unknown_extension_classifies_as_other() {
        // 【テスト目的】: 未知拡張子（.bin）が拒否されず、file_type=otherとして受理されることを確認する
        // 【テスト内容】: filename="example.bin"のmultipartで実在itemへPOSTする
        ...
        let (content_type, body) = multipart_body(b"dummy bytes", "example.bin", None);
        ...
        assert_eq!(response.status(), StatusCode::CREATED); // 【確認内容】: 未知拡張子がotherとして201で受理されることを確認 🟡
    }
    ...
    async fn post_item_file_upload_without_file_field_returns_400() {
        ...
        let (content_type, body) = multipart_body_without_file();
    ...
    async fn post_item_file_upload_with_invalid_uuid_path_returns_400() {
        ...
        let (content_type, body) = multipart_body(b"dummy bytes", "example.pdf", None);
    ...
    async fn post_item_file_upload_with_empty_file_returns_201() {
        ...
        let (content_type, body) = multipart_body(b"", "empty.pdf", None);
    ...
    async fn post_item_file_with_existing_item_returns_201() {
        ...
        let item_id: Uuid = sqlx::query_scalar(
            "INSERT INTO items (media_type, title, source) VALUES ('novel', 'テスト書籍', 'manual') RETURNING id",
        )
        .fetch_one(&state.db)
        .await
        .expect("テスト用item作成に失敗しました");
