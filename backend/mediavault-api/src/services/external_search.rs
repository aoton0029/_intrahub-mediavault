...
use crate::models::item_image::{ImageKind, ImageSource, NewItemImage};
...
/// 画像一覧からURL重複を除去する（順序維持・先勝ち。kind/sourceは先に現れた要素を採用する）。
///
/// 【設計方針】: 旧実装の「キー名ヒントによる生JSON再帰走査（extract_image_urls）」は
/// 同一画像のサイズ・フォーマット違いや無関係画像（アイコン等）まで拾うノイズ源だったため廃止し、
/// 各プロバイダの`build_*_create_request`がレスポンス構造に基づき`(url, kind, source)`を
/// 明示的に組み立てる方式へ変更した。
fn dedup_images(images: Vec<NewItemImage>) -> Vec<NewItemImage> {
...
fn build_anime_create_request(work: &WorkModel, jikan_data: Option<&Value>) -> CreateItemRequest {
    ...
    // Annictのカバー候補（recommended/OGP）＋Jikanの大判カバーのみを明示的に採用する
    // （Jikanレスポンス全走査はjpg/webp×サイズ違いの同一画像を大量に拾うためやめた）
    let mut collected_images: Vec<NewItemImage> = Vec::new();
    if let Some(images) = &work.images {
        if let Some(url) = non_empty(images.recommended_url.clone()) {
            collected_images.push(NewItemImage::new(
                url,
                ImageKind::Cover,
                ImageSource::Annict,
            ));
    ...
    if let Some(url) = jikan_data.and_then(jikan_large_image_url) {
        collected_images.push(NewItemImage::new(url, ImageKind::Cover, ImageSource::Jikan));
    }
    let additional_images = dedup_images(collected_images);
...
pub(crate) fn build_book_create_request(
    ...
    // サイズ違いは同一画像のため、最大サイズ1枚のみをカバーとして採用する
    let additional_images = book
        .large_image_url
        .clone()
        .or_else(|| book.medium_image_url.clone())
        .or_else(|| book.small_image_url.clone())
        .map(|url| NewItemImage::new(url, ImageKind::Cover, ImageSource::Rakuten))
        .into_iter()
        .collect();
...
fn build_movie_create_request(data: &Value) -> CreateItemRequest {
    ...
    let additional_images = dedup_images(
        [
            tmdb_image_url(data, "poster_path")
                .map(|url| NewItemImage::new(url, ImageKind::Cover, ImageSource::Tmdb)),
            tmdb_image_url(data, "backdrop_path")
                .map(|url| NewItemImage::new(url, ImageKind::Backdrop, ImageSource::Tmdb)),
        ]
        .into_iter()
        .flatten()
        .collect(),
...
fn build_game_create_request(data: &Value) -> CreateItemRequest {
    ...
    // header_image（カバー）＋スクリーンショットのフルサイズのみを採用する
    // （capsule各サイズやpath_thumbnail等の縮小バリアントは保存しない）
    let screenshot_full_urls: Vec<String> = data
        .get("screenshots")
        .and_then(Value::as_array)
        .map(|arr| {
            arr.iter()
                .filter_map(|s| json_str(s, "path_full"))
                .collect()
        })
        .unwrap_or_default();
    let additional_images = dedup_images(
        json_str(data, "header_image")
            .map(|url| NewItemImage::new(url, ImageKind::Cover, ImageSource::Steam))
            .into_iter()
            .chain(
                screenshot_full_urls
                    .into_iter()
                    .map(|url| NewItemImage::new(url, ImageKind::Screenshot, ImageSource::Steam)),
            )
            .collect(),
    );
...
fn build_novel_create_request(item: &NdlItemModel, media_type: MediaType) -> CreateItemRequest {
    ...
    let additional_images = item
        .thumbnail_url
        .clone()
        .map(|url| NewItemImage::new(url, ImageKind::Cover, ImageSource::Ndl))
        .into_iter()
        .collect();
