...
export function MangaDetailPage() {
  ...
  if (detail.isLoading) {
    return <EmptyState title="読み込み中です" description="漫画詳細を取得しています。" />;
  }
    ...
    <DetailLayout
      ...
      main={(
        <DetailMain
          ...
          propertyList={detailSectionMatrix.manga.propertyList ? detail.propertyItems : undefined}
          groups={detailSectionMatrix.manga.groupList ? detail.groups : undefined}
          groupTitle="巻構成"
          groupActions={(group) => (
            <InlineAddForm
              triggerLabel="話数を追加"
          ...
          groupFooter={(
            <InlineAddForm
              triggerLabel="巻を追加"
              fields={[{ name: "name", placeholder: "巻名", defaultValue: `${detail.groups.length + 1}巻` }]}
              onSubmit={(values) => {
                if (!values.name) return;
                void runAction(() => detail.addGroup(values.name, detail.groups.length + 1), "巻を追加しました。");
              }}
            />
          )}
