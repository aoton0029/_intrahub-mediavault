...
async function fetchAnimeDetailBundle(id: string): Promise<AnimeDetailBundle> {
  const [item, groups, staff, cast, relations, streamingLinks, images, mylists, links, files, trailers, citations] = await Promise.all([
  ...
  const episodePairs = await Promise.all(
    groups.map(async (group) => [
      group.id,
      await fetchApi<ItemEpisode[]>(`/groups/${group.id}/episodes`),
    ] as const),
  );
...
export function useAnimeDetailData(id: string | undefined) {
  const queryClient = useQueryClient();
  const queryKey = ["anime-detail", id] as const;

  const detailQuery = useQuery({
    queryKey,
    enabled: Boolean(id),
    queryFn: () => fetchAnimeDetailBundle(id!),
  });
  ...
  const categoryAddMutation = useMutation({
    mutationFn: async (name: string) => {
    await fetchApi(`/items/${id}/categories`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name }),
    });
  ...
  const groupAddMutation = useMutation({
    ...
    await fetchApi(`/items/${id}/groups`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ group_type: "season", group_name: name, number, display_order: number ?? 0 }),
    });
    },
    onSuccess: invalidate,
  });
...
export type UseAnimeDetailDataResult = ReturnType<typeof useAnimeDetailData>;
export { mapGroup, STREAMING_PLATFORM_LABELS };
