...
| ファイルアクセス | `/data`（`MEDIA_ROOT`/`DOCUMENTS_ROOT`） |
| デプロイ | Docker |
...
| `/data`（`MEDIA_ROOT`/`DOCUMENTS_ROOT`） | ファイル処理 | 必須 |
| LLMエンドポイント（`LLM_BASE_URL`/`LLM_API_KEY`） | `wiki`/`embed`をworker自身が実行する場合のみ | オプション（既定はKnowledgeHub側エージェントに委譲） |

## リトライ方針
ジョブは状態（state）・リトライ回数を `jobs` テーブルで管理し、失敗時は再試行する。具体的なリトライ回数・バックオフ方針は実装時に定める。
