...
- ルート: `/media/:id`（`mediaType: drama`）。パンくずは「一般メディア / ドラマ」。編集ボタンあり
...
```ts
interface DramaDetail extends ItemDetailBase {
  mediaType: 'drama';
  detail: {
    networks: string;
    originalLanguage: string;
    firstAirDate: string;
    lastAirDate?: string;      // 放送中は undefined → 「放送中」表示
...
モックは2シーズン分の `group-block` を表示（各シーズンのheaderに「(全n話)」を含む）。`GroupList` はグループを複数件レンダーできる設計とする。

## API連携（映画からの差分のみ）

- 種別固有情報フィールドが異なる以外は `20_movie_detail.md` §6と同一
- シーズン構成のAPIは `16_anime_detail.md` と同一（`group_type: season`）
