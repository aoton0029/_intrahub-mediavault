...
本画面は `00_common.md` §5「詳細画面共通パターン」に準拠するが、`rail-facts` がインタラクティブでなく **静的表示** である点がメディア詳細（16, 20-24）と異なる（下記参照）。
...
- タグ/カテゴリに削除ボタン（×）が無い（`.tag-pill` のみ、`.tag-remove`/`.tag-add-trigger` が無い）。同様に読み取り専用の可能性が高いが、**【要確認】**メディア詳細と挙動を揃えるべきか、モック通り読み取り専用にすべきかはUX方針次第
...
- タグ/カテゴリ: 【要確認】読み取り専用なら追加/削除APIは呼ばない。メディアと同様の編集可能UIにする場合は `20_movie_detail.md` と同じ `POST/DELETE /items/{id}/tags`・`/items/{id}/categories`
