...
- **プログラミング言語**: TypeScript 5.7+ / React 18.3+（TSX）
  - **言語選択の理由**: プロジェクト全体が React + TypeScript で統一されており、props インターフェースを型安全に検証できる（note.md「1. 技術スタック」）。
  - **テストに適した機能**: 判別共用体（Item型）・厳密な型チェックにより、テストフィクスチャの型整合性をコンパイル時に保証できる。
  - 🔵 信頼性: note.md「1. 技術スタック」「8. 注意事項・制約 > 型安全性」より確実。
- **テストフレームワーク**: Vitest + @testing-library/react + @testing-library/jest-dom（環境: jsdom）
  - **フレームワーク選択の理由**: `frontend/vitest.config.ts` で Vitest（globals: true, jsdom）が設定済み、`src/test/setup.ts` で jest-dom matchers が有効化済み（note.md「5. テスト関連情報」）。
  - **テスト実行環境**: jsdom（ブラウザDOM相当）。`yarn test` で1回実行、`yarn test:watch` でウォッチ。
  - 🔵 信頼性: vitest.config.ts, src/test/setup.ts, App.test.tsx より確実。
...
🔵 信頼性: note.md「2. 開発ルール > テスト駆動開発」「8. 注意事項・制約」より確実。

### 共通テストフィクスチャ（Item型）

Item は `media_type` による判別共用体で、`details`・`id`・`source`・`status`・`isFavorite`・`createdAt`・`updatedAt` が必須（interfaces.ts L125-L152）。MediaCard テストでは以下のような最小フィクスチャを用いる想定。

```typescript
// 【テストデータ準備】: Item判別共用体の必須フィールドを満たす最小フィクスチャ生成ヘルパー
// 【初期条件設定】: mediaTypeごとにdetailsの必須配列フィールド（genreList等）を満たす
// 🔵 interfaces.ts ItemBase/各Detailsの必須フィールドより
...
**参照**: requirements.md「2.1 MediaCard」, note.md「6. 実装対象コンポーネント > MediaCard」, interfaces.ts ItemBase
...
- 🟡 信頼性: requirements.md「4.4 エラーケース（入力欠落で描画を破綻させない）」から妥当な推測
...
- 🟡 信頼性: requirements.md「4.3 MediaCard: coverImageUrl 欠落」から妥当な推測
...
- **入力値**: 8種別それぞれの最小 Item（`it.each` で `['anime','movie','drama','manga','novel','game','academic_book','paper']` を反復）
  - **境界値選択の根拠**: MediaType の取りうる全値（interfaces.ts L19-27, media-type-accent.ts の8キー）
...
- 🔵 信頼性: interfaces.ts MediaType（8種別）, requirements.md「4.3 8種別網羅」より確実
...
- **テストの目的**: 文字列境界での堅牢性確認
  - **堅牢性の確認**: 空入力に対する防御
- 🟡 信頼性: interfaces.ts で title は必須 string。空文字の扱いは設計文書に明記なし（妥当な推測）
...
**参照**: requirements.md「2.2 MediaTypeBadge」, media-type-accent.ts, note.md「6 > MediaTypeBadge」
...
- **期待される結果**: `screen.getByText('アニメ')` が存在し、その要素（または親バッジ）が `className` に `text-accent-anime` を含む
  - **期待結果の理由**: requirements.md「2.2 出力」+ media-type-accent.ts L12（anime→text-accent-anime）
- **テストの目的**: ラベル変換とアクセント色適用の両立確認
  - **確認ポイント**: getMediaTypeAccentClass の戻り値が実際に className へ反映されること
- 🔵 信頼性: media-type-accent.ts（クラス確実）+ 🟡（日本語ラベル「アニメ」は requirements.md「2.2」の妥当な推測）
...
- 🔵 信頼性: media-type-accent.ts L11-20, requirements.md「3. 制約条件」より確実
...
- **入力値**: `mediaType={'unknown' as MediaType}`
  - **不正な理由**: MediaType の8値に含まれず、`getMediaTypeAccentClass` が undefined を返し得る
...
- **入力値**: `it.each` で8種別を反復。各種別と期待クラスの対応表:
  ...
  - **境界値選択の根拠**: media-type-accent.ts `MEDIA_TYPE_ACCENT_CLASS` の8キー全件（L11-20）
...
- 🔵 信頼性: media-type-accent.ts L11-20（8キー・クラス名確実）, requirements.md「4.3 8種別網羅」より確実
...
- 🟡 信頼性: requirements.md「4.3 FilterBar: children 未指定」より妥当な推測
...
- **テスト名**: EmptyState は actionLabel 指定時にアクションボタンを表示する
  - **何をテストするか**: `actionLabel` が指定されたときボタンが描画されること
...
- **期待される結果**: `screen.getByRole('button', { name: 'アイテムを追加' })` が存在する
  - **期待結果の理由**: requirements.md「2.4 出力（actionLabel 指定時はボタンを表示）」より
...
- **入力値**: `actionLabel='追加'`（`onAction` なし）、`userEvent.click(button)`
...
- 🟡 信頼性: requirements.md「2.4 入出力の関係性（ペアで機能）」からの妥当な推測
...
- 🟡 信頼性: requirements.md「4.3 EmptyState: action 未指定（メッセージのみ表示）」より妥当な推測
...
- **テストの目的**: 文字列境界での堅牢性確認
  - **堅牢性の確認**: 空メッセージへの防御
- 🟡 信頼性: interfaces.ts/requirements で message は必須 string。空文字扱いは明記なし（妥当な推測）
...
- **入力値**: `open=false`, `title='削除しますか？'`, `onConfirm/onCancel=vi.fn()`
  - **境界値選択の根拠**: open の取りうる2値のうち非表示側。requirements.md「4.3 open=false」
...
- 🟡 信頼性: requirements.md「2.5」「4.3 ConfirmDialog: open=false」より妥当な推測
...
| TC-ES-N-02 | EmptyState | 正常系 | actionLabel でボタン表示 | 🟡 |
...
- 🔵 青信号: 10件（33%）— title 表示・onClick・MediaTypeBadge 委譲・8種別網羅・children パススルー・accent クラス・message 表示
- 🟡 黄信号: 18件（60%）— レイアウト詳細・日本語ラベル・EmptyState/ConfirmDialog の挙動・任意 prop 省略時の堅牢性
...
- **参照したEARS要件**: REQ-001（一覧カード表示→MediaCard）, REQ-002（絞り込み→FilterBar枠）, REQ-007（削除→ConfirmDialog）, EDGE-101（0件→EmptyState）
- **参照した設計文書**: architecture.md「コンポーネント粒度」, interfaces.ts（Item 判別共用体, MediaType, ItemStatus, ItemBase）, media-type-accent.ts（getMediaTypeAccentClass, MEDIA_TYPE_ACCENT_CLASS）, button.tsx（cva/cn 参考）, App.test.tsx（テスト参考パターン）
...
```
✅ 高品質:
- テストケース分類: 正常系（15）・異常系（5）・境界値（10）を5コンポーネント全てで網羅
- 期待値定義: 各ケースで具体的な入力値・期待アサーション（getByText/getByRole/toHaveBeenCalled 等）を明示
- 技術選択: TypeScript + Vitest + @testing-library/react（jsdom）で確定（設定ファイル既存）
- 実装可能性: 依存（media-type-accent.ts, utils.ts, shadcn Button）は既存。Dialog/Badge は shadcn 追加で導入可能
- 信頼性レベル: 🔵10 / 🟡18 / 🔴2。🔴2件は設計文書外の防御的ケースで、実装方針により調整可能（TDDの阻害要因なし）
```
